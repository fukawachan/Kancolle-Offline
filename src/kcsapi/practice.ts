...
export function generatePracticeBatch(periodKey = practicePeriodKey(), generatedAt = Date.now()): PracticeBatch {
  const rng = new BattleRng(seedFromPeriod(periodKey));
  return {
    periodKey,
    generatedAt: Math.trunc(generatedAt),
    rivals: Array.from({ length: RIVAL_COUNT }, (_value, index) => practiceRival(index + 1, rng))
  };
}

export function practiceRivalById(rivals: PracticeRival[], id: number) {
  return cloneRival(rivals.find((rival) => rival.id === id) ?? rivals[0] ?? practiceRival(1, new BattleRng(1)));
}
...
function practiceRival(id: number, rng: BattleRng): PracticeRival {
  const level = rng.intInclusive(80, 120);
  const shipCount = rng.chance(0.5) ? 2 : rng.intInclusive(3, 6);
  return {
    id,
    ...
    ships: practiceShips(id, shipCount, rng)
  };
}

function practiceShips(rivalId: number, count: number, rng: BattleRng) {
  const pool = [...PRACTICE_SHIP_POOL];
