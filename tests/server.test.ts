...
describe("local Fastify server", () => {
  ...
  it("serves cache-backed slot item-up art when the exact equipment art is unavailable", async () => {
    const app = await buildApp({
      cacheDir: path.resolve("cache"),
      stateStore: store,
      unknownLogPath: path.join(tempDir, "unknown.jsonl")
    ...
    const response = await app.inject({ method: "GET", url: "/kcs2/resources/slot/item_up/0570_1119.png?version=2" });
